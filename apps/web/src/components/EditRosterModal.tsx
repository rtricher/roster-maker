...
interface EditRosterModalProps {
  ...
  detachment: string
  maxPoints: number
  onSave: (name: string, faction: string, detachment: string, maxPoints: number) => void
  onCancel: () => void
}

export default function EditRosterModal({ name: initName, faction: initFaction, detachment: initDetachment, maxPoints: initMaxPoints, onSave, onCancel }: EditRosterModalProps) {
  const [name, setName] = useState(initName)
  const [faction, setFaction] = useState(initFaction)
  const [detachment, setDetachment] = useState(initDetachment)
  const [maxPoints, setMaxPoints] = useState(initMaxPoints)

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!name.trim()) return
    onSave(name.trim(), faction.trim(), detachment.trim(), maxPoints)
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onCancel}>
      ...
      >
        <h2 className="text-lg font-bold text-gray-100 mb-4">Edit Roster</h2>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              className="w-full mt-1 bg-surface-900 border border-surface-600 rounded px-3 py-2 text-gray-100 focus:outline-none focus:border-olive-500"
            />
            <p className="text-xs text-gray-600 mt-1">This is a soft cap — points are not enforced</p>
          </div>
        </div>
